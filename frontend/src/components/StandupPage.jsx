...
const StandupPage = ({ standupList, setStandupList }) => {
  ...
  const connectToRoom = (roomUUID) => {
    if (sockets.current[roomUUID]) return; // already connected

     const token = localStorage.getItem(ACCESS_TOKEN) || sessionStorage.getItem(ACCESS_TOKEN);
     
    const socket = new WebSocket(`${import.meta.env.VITE_WS_URL}/ws/${roomUUID}/?token=${token}`);
  ...
  const handleSendMessage = async (standupId) => {
    ...
    try {
      ...
      if (socket?.readyState === WebSocket.OPEN) {
        ...
        setError("Connection issue. some messages may not able to send");
      }
  ...
  const handleDeleteStandup = async (standupId) => {
    setError(null); 
    const confirmDelete = window.confirm("Are you sure you want to delete this this standup?");
